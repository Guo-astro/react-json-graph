...
import { JsonDataType } from "../components/JsonGraph/Nodes/json.types";
import {
  type ObjectCustomNode,
  NodeType,
  type ArrayCustomNode,
  type PrimitiveCustomNode,
  type CustomNode,
} from "../components/JsonGraph/Nodes/node.types";
import { getXYPosition } from "./graph.utils";
import { getJsonDataType, validateJsonDataType } from "./json.utils";
...
type DefaultEdgeParams = SourceTarget & {
...
type TraverseParams = {
  ...
  depth: number;
  arrayIndexForObject: number | null;
  sourceSet: { source?: string; sourceHandle?: string | undefined };
  parentNodePathIds: string[];
};
...
type TraverseArrayParams = {
  ...
  _parentNodePathIds: string[];
  _source: string;
  _sourceHandle?: string | undefined;
};
