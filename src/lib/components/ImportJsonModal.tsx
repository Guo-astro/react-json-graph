  ...
  useEffect(() => {
    async function handleData() {
      if (isObject(data) || isArray(data)) {
        setIsValidJson(true);
        setJsonTree(convertJsonTree(data as object | unknown[]));
        const contents = await jsonToContent(data as object, format);
    ...
    <Modal isOpen={isModalOpen} onClose={closeModal}>
      ...
      <div className="w-full overflow-y-auto p-2 overflow-x-hidden">
        <div className="w-full flex flex-col landscape:max-md:flex-row items-center md:flex-row justify-between gap-2   ">
          ...
          />
          <button
            className={`flex items-center gap-1 font-semibold px-2 py-1 rounded-md cursor-not-allowed dark:border-2 bg-lightBackground text-white dark:bg-darkBackground dark:text-lightText dark:border-gray-700  ${
              !!url && " !bg-blue-500   dark:text-white cursor-pointer "
            } `}
